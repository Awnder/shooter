...
# Q-Learning and neural network variables.
gamma = 0.99              # discount factor highly values future reward
epsilon = 1.0             # begin with 100% random exploration
epsilon_decay = 0.995     # decay slowly
min_epsilon = 0.05        # never fully stop exploring
target_update_freq = 10   # how often to reload target_net from q_net
batch_size = 64           # balance learning with efficiency
max_episodes = 300

class DQN(nn.Module):
    '''
    Simple fully-connected forward-feed MLP network.
    '''
...
class DeepQAgent:
    def train_episode(env, online_net, stable_net, replay, optimizer):
        # init episode w/ observation being input to nn
    ...
    def train_dqn():
        """
        Train a Deep Q-Learning neural network to play our shooter game.
        """
        global epsilon
        ...
        for episode in range(1, max_episodes):
            ...
            if episode > 0 and episode % 1000 == 0:
                print(f"\n> Saving agent at episode {episode} <<\n")
